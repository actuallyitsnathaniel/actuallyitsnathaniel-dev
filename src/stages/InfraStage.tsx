...
const ROWS: InfraRow[] = [
  {
    status: "green", name: "performance + seo", stat: "operational",
  ...
  {
    status: "green", name: "audience tooling", stat: "operational",
  ...
  {
    status: "green", name: "media pipeline", stat: "operational",
  ...
  {
    status: "green", name: "open-source tooling", stat: "operational",
  ...
  {
    status: "green", name: "ci / cd", stat: "operational",
    ...
    aliases: ["cicd", "ci", "cd", "deploy", "deployment", "pipeline", "jenkins", "harness"],
  },
  {
    status: "green", name: "observability", stat: "operational",
    ...
    aliases: ["observability", "monitoring", "alerts", "newrelic", "splunk", "logs"],
  },
];
...
export function InfraStage({ filter }: InfraStageProps) {
  ...
  return (
    <div>
      <h2>infra</h2>
      <div className="lede">systems i've shipped and maintained. metrics are real.</div>
      <div className="status-board" role="table" aria-label="infrastructure status">
            ...
            >
              ...
              <div className="sb-metric">
                <span className={`m-v${row.metric.qualitative ? " m-q" : ""}`}>
                  {row.metric.v}
                  {row.metric.su && <span className="m-su"> {row.metric.su}</span>}
                </span>
      ...
      <p className="sb-foot">
        ...
        all metrics are real and defensible. enterprise rows show qualitative status instead of fabricated p99s.
      </p>
    </div>
  );
}
