...
export function LogOverlay({ open, onClose }: LogOverlayProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-bg1 border border-rule2 rounded-sm w-[min(560px,92vw)] px-7 py-6 max-h-[70vh] overflow-y-auto flex flex-col gap-0"
      ...
      >
        ...
        {logs.length === 0 ? (
          ...
          <div className="flex flex-col gap-0 font-mono text-[11px] leading-[1.7]">
            {logs.map((entry) => (
              <div
                key={entry.id}
                className="grid grid-cols-[60px_64px_1fr] gap-2.5 py-0.5"
              >
        ...
        <div className="mt-3.5 pt-3 border-t border-rule text-faint text-[11px] text-center shrink-0">
          esc to close
        </div>
      </div>
    </div>
